...
namespace AssetManager
{
	public class WindowList : IDisposable
	{
		...
		private void BuildWindowList(ExtendedForm parentForm, ref ToolStripItemCollection targetMenuItem)
		{
			foreach (ExtendedForm frm_loopVariable in ChildFormControl.GetChildren(parentForm)) {
				...
				if (HasChildren(frm)) {
					...
					BuildWindowList(frm, ref NewDropDown.DropDownItems);
				} else {
					if (frm is SibiMainForm) {
		...
		private bool HasChildren(ExtendedForm parentForm)
		{
			...
			if (Children.Count == 0) {
				...
				foreach (ExtendedForm frm_loopVariable in Children) {
					frm = frm_loopVariable;
					if (!frm.IsDisposed) {
						return true;
					}
		...
		private void RefreshWindowList()
		{
			...
			if (NumOfForms != intFormCount) {
				if (!DropDownControl.DropDown.Focused) {
					...
					AddParentMenu();
					BuildWindowList(MyParentForm, ref DropDownControl.DropDownItems);
