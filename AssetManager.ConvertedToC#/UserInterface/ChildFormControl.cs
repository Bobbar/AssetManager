...
namespace AssetManager
{
	public static class ChildFormControl
	{
		...
		{
			foreach (ExtendedForm frm_loopVariable in GetChildren(parentForm)) {
		...
		public static void LookupDevice(ExtendedForm parentForm, DeviceObject device)
		{
			if (device.GUID != null) {
				...
				Message("Device not found.", Constants.vbOKOnly + Constants.vbExclamation, "Error", parentForm);
			}
		}

		public static void MinimizeChildren(ExtendedForm parentForm)
		{
			foreach (ExtendedForm child_loopVariable in GetChildren(parentForm)) {
