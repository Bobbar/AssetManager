	...
	public partial class SibiMainForm
	{
		...
		private void InitForm()
		{
			try {
				...
				this.GridTheme = new GridTheme(AssetManager.Colors.Colors.HighlightBlue, AssetManager.Colors.Colors.SibiSelectColor, ResultGrid.DefaultCellStyle.BackColor);
				ToolStrip1.BackColor = AssetManager.Colors.Colors.SibiToolBarColor;
		...
		{
			var cmd = DBFactory.DBFactory.GetDatabase().GetCommand();
		...
		private void ExecuteCmd(ref DbCommand cmd)
		{
			try {
				LastCmd = cmd;
				SendToGrid(DBFactory.DBFactory.GetDatabase().DataTableFromCommand(cmd));
			} catch (Exception ex) {
		...
		public void SendToGrid(DataTable results)
		{
			try {
				using (results) {
					...
					AssetManager.GridFunctions.GridFunctions.PopulateGrid(ResultGrid, results, SibiTableColumns());
		...
		private void SetDisplayYears()
		{
			...
			using (DataTable results = DBFactory.DBFactory.GetDatabase().DataTableFromQueryString(strQRY)) {
		...
		private void ShowAll(string Year = "")
		{
			...
			if (Year == "All") {
                ...
                newCommand = DBFactory.DBFactory.GetDatabase().GetCommand("SELECT * FROM " + SibiRequestCols.TableName + " ORDER BY " + SibiRequestCols.RequestNumber + " DESC");
                ExecuteCmd(ref newCommand);//DBFactory.DBFactory.GetDatabase().GetCommand("SELECT * FROM " + SibiRequestCols.TableName + " ORDER BY " + SibiRequestCols.RequestNumber + " DESC"));

            } else {
                DbCommand newCommand;
                newCommand = DBFactory.DBFactory.GetDatabase().GetCommand("SELECT * FROM " + SibiRequestCols.TableName + " WHERE " + SibiRequestCols.DateStamp + " LIKE '%" + Year + "%' ORDER BY " + SibiRequestCols.RequestNumber + " DESC");
                ExecuteCmd(ref newCommand);//DBFactory.DBFactory.GetDatabase().GetCommand("SELECT * FROM " + SibiRequestCols.TableName + " WHERE " + SibiRequestCols.DateStamp + " LIKE '%" + Year + "%' ORDER BY " + SibiRequestCols.RequestNumber + " DESC"));
		...
		private void ResultGrid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
		{
			if (ResultGrid.CurrentRow.Index > -1)
				OpenRequest(AssetManager.GridFunctions.GridFunctions.GetCurrentCellValue(ResultGrid, SibiRequestCols.UID));
		}
		...
		{
			foreach (StatusColumnColorStruct status_loopVariable in StatusColors) {
				status = status_loopVariable;
		...
		private void HighlightCurrentRow(int Row)
		{
			try {
				if (!bolGridFilling) {
					StyleFunctions.HighlightRow(ref ResultGrid, GridTheme, Row);
				}
			} catch {
			}
		}
		...
		{
			StyleFunctions.LeaveRow(ref ResultGrid, GridTheme, e.RowIndex);
		}
