...
namespace AssetManager
{
	...
	public partial class ViewHistoryForm
	{
		...
		private List<Control> GetChangedFields(DataTable currentData)
		{
			...
			using (olderData == AssetManager.DBFactory.GetDatabase().DataTableFromQueryString(Query)) {
				if (olderData.Rows.Count > 0) {
					...
					foreach (string col_loopVariable in ChangedColumns) {
		...
		private void HighlightChangedFields(DataTable currentData)
		{
			...
			foreach (Control ctl_loopVariable in GetChangedFields(currentData)) {
				ctl = ctl_loopVariable;
				ctl.BackColor = AssetManager.Colors.Colors.CheckIn;
			}
