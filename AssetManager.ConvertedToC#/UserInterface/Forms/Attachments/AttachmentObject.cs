...
namespace AssetManager
{
	...
	public class Attachment : IDisposable
	{
		...
		public Attachment(DataTable attachInfoTable, AttachmentsBaseCols attachTable)
		{
			...
			_attachTable = attachTable;
			var _with1 = TableRow;
			_fileName = _with1.Item(attachTable.FileName).ToString;
			_fileUID = _with1.Item(attachTable.FileUID).ToString;
			_MD5 = _with1.Item(attachTable.FileHash).ToString;
			_fileSize = Convert.ToInt32(_with1.Item(attachTable.FileSize));
			_extention = _with1.Item(attachTable.FileType).ToString;
			_folderName = _with1.Item(attachTable.Folder).ToString;
			_folderGUID = _with1.Item(attachTable.FKey).ToString;
		}
		...
		public Attachment(DataTable attachInfoTable, string selectedFolder, AttachmentsBaseCols attachTable)
		{
			...
			_attachTable = attachTable;
			var _with2 = TableRow;
			_fileName = _with2.Item(attachTable.FileName).ToString;
			_fileUID = _with2.Item(attachTable.FileUID).ToString;
			_MD5 = _with2.Item(attachTable.FileHash).ToString;
			_fileSize = Convert.ToInt32(_with2.Item(attachTable.FileSize));
			_extention = _with2.Item(attachTable.FileType).ToString;
			_folderName = _with2.Item(attachTable.Folder).ToString;
			_folderGUID = _with2.Item(attachTable.FKey).ToString;
		}
		...
		private string GetHash(FileInfo Fileinfo)
		{
			using (FileStream HashStream = Fileinfo.OpenRead()) {
				return AssetManager.SecurityTools.SecurityFunctions.GetMD5OfStream(HashStream);
			}
		}
