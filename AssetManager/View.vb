...
Public Class View
    ...
    Private Sub GetNewValues()
        With NewData
            ...
            .strCurrentUser = Trim(txtCurUser_View.Text)
            .dtPurchaseDate = dtPurchaseDate_View.Value.ToString(strDBDateFormat)
            Debug.Print(dtPurchaseDate_View.Value.ToString(strDBDateFormat))
            .strReplaceYear = Trim(txtReplacementYear_View.Text)
            .strOSVersion = GetDBValue(ComboType.OSType, cmbOSVersion.SelectedIndex)
        End With
    End Sub
    ...
    Public Sub ViewDevice(ByVal DeviceUID As String)
        ...
        With reader
            Do While .Read()
                CollectDeviceInfo(!dev_UID,!dev_description,!dev_location,!dev_cur_user,!dev_serial,!dev_asset_tag,!dev_purchase_date,!dev_replacement_year,!dev_po,!dev_osversion,!dev_eq_type)
        ...
        DataGridHistory.Rows(0).DefaultCellStyle.BackColor = colCurrentEntry
        DisableControls()
    End Sub
    ...
    Public Sub UpdateDevice()
        ...
        cn_global.Open()
        'Dim strSqlQry1 = "UPDATE devices (dev_description,dev_location,dev_cur_user,dev_serial,dev_asset_tag,dev_purchase_date,dev_replacement_year,dev_eq_type) VALUES ('" & NewData.strDescription & "','" & NewData.strLocation & "','" & NewData.strCurrentUser & "','" & NewData.strSerial & "','" & NewData.strAssetTag & "','" & NewData.dtPurchaseDate & "','" & NewData.strReplaceYear & "','" & NewData.strEqType & "')"
        Dim strSQLQry1 = "UPDATE devices set dev_description='" & NewData.strDescription & "', dev_location='" & NewData.strLocation & "', dev_cur_user='" & NewData.strCurrentUser & "', dev_serial='" & NewData.strSerial & "', dev_asset_tag='" & NewData.strAssetTag & "', dev_purchase_date='" & NewData.dtPurchaseDate & "', dev_replacement_year='" & NewData.strReplaceYear & "', dev_osversion='" & NewData.strOSVersion & "', dev_eq_type='" & NewData.strEqType & "' WHERE dev_UID='" & CurrentDevice.strGUID & "'"
        Debug.Print(strSQLQry1)
        Dim cmd As New MySqlCommand
        cmd.Connection = cn_global
        cmd.CommandText = strSQLQry1
        cmd.ExecuteNonQuery()
        Dim strSqlQry2 = "INSERT INTO historical (hist_change_type,hist_notes,hist_serial,hist_description,hist_location,hist_cur_user,hist_asset_tag,hist_purchase_date,hist_replacement_year,hist_osversion,hist_dev_UID,hist_action_user,hist_eq_type) VALUES ('" & GetDBValue(ComboType.ChangeType, UpdateDev.cmbUpdate_ChangeType.SelectedIndex) & "','" & strNote & "','" & NewData.strSerial & "','" & NewData.strDescription & "','" & NewData.strLocation & "','" & NewData.strCurrentUser & "','" & NewData.strAssetTag & "','" & NewData.dtPurchaseDate & "','" & NewData.strReplaceYear & "','" & NewData.strOSVersion & "','" & CurrentDevice.strGUID & "','" & strLocalUser & "','" & NewData.strEqType & "')"
        cmd.CommandText = strSqlQry2
        cmd.ExecuteNonQuery()
        cn_global.Close()
        Dim blah
        blah = MsgBox("Update Added", vbOKOnly, "Complete")
        ViewDevice(CurrentDevice.strAssetTag)
    End Sub
